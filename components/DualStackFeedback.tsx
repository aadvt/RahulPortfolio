...
export default function DualStackFeedback() {
  ...
  useEffect(() => {
    ...
    // Initial state:
    // Card 0 = active at z=0, scale=1, rotateY=0, opacity=1
    // Card 1+ = queued behind at opacity=0
    ...
    for (let i = 0; i < total - 1; i++) {
      ...
      // 1. Active card flips Y and moves to the opposite side
      tl.to(fromImg, { rotateY: isOddTransition ? -180 : 180, x: () => (isOddTransition ? 0 : offsets.x), y: () => (isOddTransition ? 0 : offsets.y), z: -80, scale: 0.86, ease: "power2.inOut", duration: seg * 0.7 }, start);
      tl.to(fromImg, { opacity: 0, duration: seg * 0.2, ease: "power1.in" }, start + seg * 0.3);
      tl.to(fromTxt, { rotateY: isOddTransition ? 180 : -180, x: () => (isOddTransition ? 0 : -offsets.x), y: () => (isOddTransition ? 0 : -offsets.y), z: -80, scale: 0.86, ease: "power2.inOut", duration: seg * 0.7 }, start + 0.07);
      tl.to(fromTxt, { opacity: 0, duration: seg * 0.2, ease: "power1.in" }, start + 0.07 + seg * 0.3);

      // 2. Next card rises from depth to front
      tl.fromTo(toImg, { rotateY: isOddTransition ? 180 : -180, x: () => (isOddTransition ? offsets.x : 0), y: () => (isOddTransition ? offsets.y : 0), z: -((total - 1 - i) * 18), scale: 0.88, opacity: 0 }, { rotateY: 0, x: () => (isOddTransition ? 0 : offsets.x), y: () => (isOddTransition ? 0 : offsets.y), z: 0, scale: 1, ease: "power2.inOut", duration: seg * 0.8 }, start + seg * 0.1);
      tl.to(toImg, { opacity: 1, duration: seg * 0.2, ease: "power1.out" }, start + seg * 0.1 + seg * 0.15);
      tl.fromTo(toTxt, { rotateY: isOddTransition ? -180 : 180, x: () => (isOddTransition ? -offsets.x : 0), y: () => (isOddTransition ? -offsets.y : 0), z: -((total - 1 - i) * 18), scale: 0.88, opacity: 0 }, { rotateY: 0, x: () => (isOddTransition ? 0 : -offsets.x), y: () => (isOddTransition ? 0 : -offsets.y), z: 0, scale: 1, ease: "power2.inOut", duration: seg * 0.8 }, start + seg * 0.1 + 0.07);
      tl.to(toTxt, { opacity: 1, duration: seg * 0.2, ease: "power1.out" }, start + seg * 0.1 + 0.07 + seg * 0.15);

      // 3. Progress bar fills proportionally
      tl.to(".dsf-progress-fill", { scaleX: (i + 1) / (total - 1), ease: "power2.inOut", duration: seg * 0.5 }, start + seg * 0.3);
    }
